...
> ⚠️ 项目**不使用 OpenFeign**：除 `pandora-gateway` 外所有业务模块严禁直接依赖 `org.springframework.cloud:spring-cloud-starter-*`。远程调用推荐：RestClient + Nacos `NamingService` 自行选址，或引入 Dubbo。
